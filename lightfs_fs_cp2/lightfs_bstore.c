...
int ftfs_bstore_meta_put_cache(DB *meta_db, DBT *meta_dbt, DB_TXN *txn,
                         struct ftfs_metadata *metadata, struct inode *dir_inode, bool is_dir)
{
	DBT value;
...
#ifdef BETR
	return ftfs_bstore_meta_put_tmp(meta_db, meta_dbt, txn, metadata, dir_inode, is_dir);
#endif
...
int ftfs_bstore_meta_del(DB *meta_db, DBT *meta_dbt, DB_TXN *txn, bool is_weak_del, bool is_dir)
{
	if (is_weak_del) {
                             ...
                             struct dir_context *ctx, struct inode *inode, struct readdir_ctx *dir_ctx)
{
	...
	//ftfs_error(__func__, "bstore!!!! dir_ctx: %px, dir_ctx->pos: %d, dir->cursor: %px\n", dir_ctx, dir_ctx->pos, dir_ctx->cursor);

	if (is_next) { // parital cache hit
		//pr_info("asdasdasdsad\n");
	...
	while (!r) {
		...
		//tmp_key = child_meta_dbt.data;
		//metap = metadata_dbt.data;
		if (!meta_key_is_child_of_ino(child_meta_key, inode->i_ino)) {
		//if (!meta_key_is_child_of_ino(tmp_key, inode->i_ino)) {
			//ftfs_error(__func__, "다른부모키!!!!!!!!!!!!!!!!!!\n");
		...
		//ino = metap->u.st.st_ino;
		//type = ftfs_get_type(metap->u.st.st_mode);
		//name = ftfs_key_path(tmp_key);
		//ftfs_error(__func__, "child_meta_key name:%s, len = %d, ino: %d, type: %d\n", name, strlen(name), ino, type);
		if (debug) {
			//pr_info("r = %d, name: %s, dis_ctx->pos :%d\n", r, name, dir_ctx->pos);
			debug = false;
		}	
		...
		//metadata_dbt.data = &meta;
		//child_meta_dbt.data = child_meta_key;
		dir_ctx->emit_cnt++;
	...
	if (r == DB_NOTFOUND) {
		...
		//ftfs_error(__func__, "다찾았다고?????\n");
		kfree(child_meta_key);
		dir_ctx->pos = 0;
		r = 0;
	} else if (r == DB_NOTFOUND_DCACHE) {
		//ftfs_error(__func__, "다는 못찾았다고?????\n");
		kfree(child_meta_key);
                      ...
                      DB_TXN *_txn, uint64_t new_num, uint64_t offset, struct inode *inode)
{
	...
	while (total_block_num) {
		...
		//BUG_ON(txn == NULL);
		//ftfs_error(__func__, "cur :%d, cnt: %d, txn: %p, txnid: %d\n", current_block_num, transfering_block_cnt, txn, txn->txn_id);
		//print_key(__func__, min_data_key_dbt.data, min_data_key_dbt.size);
...
int ftfs_bstore_scan_one_page(DB *data_db, DBT *meta_dbt, DB_TXN *txn, struct page *page, struct inode *inode)
{
...
#ifdef DATA_CHECK
	chk_buf = kmalloc(FTFS_BSTORE_BLOCKSIZE, GFP_KERNEL);
	dbt_setup(&chk_value, chk_buf, FTFS_BSTORE_BLOCKSIZE);
	...
	if (ret == -ENOENT) {
		...
		if (chk_ret == DB_NOTFOUND) {
			...
			if (memcmp(buf, chk_buf, FTFS_BSTORE_BLOCKSIZE)) {
				pr_info("ERROR\n");
				memcpy(buf, chk_buf, FTFS_BSTORE_BLOCKSIZE);
				kfree(chk_buf);
				//BUG_ON(1);
			} else {
				kfree(chk_buf);
...
int ftfs_bstore_scan_pages(DB *data_db, DBT *meta_dbt, DB_TXN *txn, struct ftio *ftio, struct inode *inode)
{
	...
	ftfs_error(__func__, "1: ftio->ft_bvidx: %d, ftio->ft_vcnt: %d block_cnt: %d, size: %d, current_block_num: %d, last_block_num: %d\n", ftio->ft_bvidx, ftio->ft_vcnt, block_cnt, size, current_block_num, last_block_num);
	block_cnt = block_cnt > last_block_num - current_block_num + 1 ? last_block_num - current_block_num + 1: block_cnt; 
	ftfs_error(__func__, "2: ftio->ft_bvidx: %d, ftio->ft_vcnt: %d block_cnt: %d, size: %d, current_block_num: %d, last_block_num: %d\n", ftio->ft_bvidx, ftio->ft_vcnt, block_cnt, size, current_block_num, last_block_num);
	/*
	if (block_cnt > 1) {
