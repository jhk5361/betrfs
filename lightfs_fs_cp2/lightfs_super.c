               ...
               struct ftfs_sb_info *sbi)
{
	...
	ret = ftfs_bstore_meta_del(sbi->meta_db, meta_dbt, txn, 0, S_ISDIR(inode->i_mode));
...
static int split_circle(struct dentry *dentry)
{
	...
	ftfs_bstore_txn_begin(sbi->db_env, NULL, &txn, TXN_MAY_WRITE);
	ret = ftfs_bstore_meta_put(sbi->meta_db, old_dbt, txn, &redirect_meta, NULL, false);
	if (ret)
		goto abort;
	ret = ftfs_bstore_meta_put(sbi->meta_db, &new_dbt, txn, &meta, NULL, false);
	if (ret)
		goto abort;
                          ...
                          struct list_head *pages, unsigned nr_pages)
{
	...
	*      case 3-1: read ahead is already issued ==> wait the issued read
...
#ifdef READA
	list_for_each_entry(ra_entry, &ftfs_inode->ra_list, list) {
		ret = buffer_processed_pages = lightfs_reada_buffer_get(ra_entry, inode, ftio, nr_pages);
	...
	/*
	buffer_processed_pages = lightfs_reada_buffer_get(inode, ftio, nr_pages);
...
#ifdef READA
	if (lightfs_reada_need(inode, ftio, nr_pages, fg_read)) {
		volatile bool is_eof = false;
		unsigned iter = 0, max_reada;
		spin_lock(&ftfs_inode->reada_spin);
		max_reada = READA_QD - ftfs_inode->ra_entry_cnt;
		while (iter < max_reada) {
			TXN_GOTO_LABEL(retry_reada);
			ftfs_bstore_txn_begin(sbi->db_env, NULL, &txn, TXN_READONLY);

			ret = ftfs_bstore_reada_pages(sbi->data_db, meta_dbt, txn, ftio, inode, iter);
					   ...
					   unsigned int flags)
{
	...
	ftfs_error(__func__, "NOTDIR1\n");
			...
			if (!ret) {
				ret = -ENOTEMPTY;
	ftfs_error(__func__, "NOTEMPTY\n");
				goto abort;
			}
		} else { // old 가 그냥 파일
			if (S_ISDIR(new_inode->i_mode)) {
				ret = -ENOTDIR;
	ftfs_error(__func__, "NOTDIR2\n");
				goto abort;
			}
		}
	}
	...
	ret = ftfs_bstore_meta_del(sbi->meta_db, old_meta_dbt, txn, 0, false);
	if (!ret)
		ret = ftfs_bstore_meta_put(sbi->meta_db, &new_meta_dbt, txn, &old_meta, new_dir, false);
...
static int ftfs_readdir(struct file *file, struct dir_context *ctx)
{
	...
	struct ftfs_sb_info *sbi = inode->i_sb->s_fs_info;
	DBT *meta_dbt = &(FTFS_I(inode)->meta_dbt);
	DB_TXN *txn;
	...
	if (ctx->pos == 2) {
		...
		dir_ctx->txn = txn;
#endif
		//ftfs_error(__func__, "dcace alloc!!!! dir_ctx: %px, dir_ctx->pos: %d, dir->cursor: %p\n", dir_ctx, dir_ctx->pos, dir_ctx->cursor);
	} else {
		dir_ctx = (struct readdir_ctx *)(ctx->pos);
		if (dir_ctx->pos == 0) { // all cache hit
			//ftfs_error(__func__, "dcache free!!!! dir_ctx: %px, dir_ctx->pos: %d, dir->cursor: %p, emit_cnt: %d\n", dir_ctx, dir_ctx->pos, dir_ctx->cursor, dir_ctx->emit_cnt);
			...
			//ftfs_error(__func__, "iter free!!!! dir_ctx: %px, dir_ctx->pos: %d, dir->cursor: %px, dir->emit_cnt: %d\n", dir_ctx, dir_ctx->pos, dir_ctx->cursor, dir_ctx->emit_cnt);
			...
			dir_ctx->txn = txn;
			dir_ctx->emit_cnt = 0;
			//ftfs_error(__func__, "iter alloc!!!! dir_ctx: %px, dir_ctx->pos: %d, dir->cursor: %px, dir->emit_cnt: %d\n", dir_ctx, dir_ctx->pos, dir_ctx->cursor, dir_ctx->emit_cnt);
		} else if (dir_ctx->pos == 3) { // end readdir
			ftfs_error(__func__, "free!!!! dir_ctx: %px, dir_ctx->pos: %d, dir->cursor: %px, dir->emit_cnt: %d\n", dir_ctx, dir_ctx->pos, dir_ctx->cursor, dir_ctx->emit_cnt);
#ifdef DISABLE_DCACHE
			dir_ctx->dcursor->c_close(dir_ctx->dcursor);
#endif
			dir_ctx->cursor->c_close(dir_ctx->cursor);
...
ftfs_fsync(struct file *file, loff_t start, loff_t end, int datasync)
{
	...
	if (ret) {
		...
		TXN_GOTO_LABEL(retry);
#ifdef GROUP_COMMIT
		ftfs_bstore_txn_begin(sbi->db_env, NULL, &txn, TXN_SYNC_WRITE);
		//ftfs_bstore_txn_begin(sbi->db_env, NULL, &txn, TXN_MAY_WRITE);
		ret = ftfs_bstore_meta_put(sbi->meta_db, meta_dbt, txn, &meta, inode, false);
#else
		ftfs_bstore_txn_begin(sbi->db_env, NULL, &txn, TXN_MAY_WRITE);
		ret = ftfs_bstore_meta_sync_put(sbi->meta_db, meta_dbt, txn, &meta, inode, false);
#endif
		if (ret) {
			DBOP_JUMP_ON_CONFLICT(ret, retry);
			ftfs_bstore_txn_abort(txn);
...
err_free_dbt:
	...
	if (strcmp(dentry->d_name.name, "prepopulated_engines.cc") == 0)
		pr_info("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!HAHA ino: %ld\n", ino);
	...
	ret = ftfs_bstore_meta_put(sbi->meta_db, &meta_dbt, txn, &meta, dir, mode & S_IFDIR);
	if (ret) {
		DBOP_JUMP_ON_CONFLICT(ret, retry);
		ftfs_bstore_txn_abort(txn);
		set_nlink(inode, 0);
...
free_meta_out:
	...
	//ftfs_bstore_txn_begin(sbi->db_env, NULL, &txn, TXN_MAY_WRITE);
	ftfs_bstore_txn_begin(sbi->db_env, NULL, &txn, TXN_SYNC_WRITE);
	ret = ftfs_bstore_meta_put(sbi->meta_db, &meta_dbt, txn, &meta, dir, false);
	//ret = ftfs_bstore_meta_sync_put(sbi->meta_db, meta_dbt, txn, &meta, inode, false);
	if (ret) {
abort:
		DBOP_JUMP_ON_CONFLICT(ret, retry);
                     ...
                     struct inode *dir, struct dentry *dentry)
{
	...
	TXN_GOTO_LABEL(retry);
	ftfs_bstore_txn_begin(sbi->db_env, NULL, &txn, TXN_MAY_WRITE);
	ret = ftfs_bstore_meta_put(sbi->meta_db, &new_meta_dbt, txn, &meta, dir, false);
	if (ret) {
		DBOP_JUMP_ON_CONFLICT(ret, retry);
		ftfs_bstore_txn_abort(txn);
	} else {
...
static int ftfs_unlink(struct inode *dir, struct dentry *dentry)
{
	...
	//TODO: handling hard_link
		...
		TXN_GOTO_LABEL(retry);
		ftfs_bstore_txn_begin(sbi->db_env, NULL, &txn, TXN_MAY_WRITE);
		ret = ftfs_bstore_meta_del(sbi->meta_db, &indirect_dbt, txn, 0, S_ISDIR(inode->i_mode));
		if (ret) {
			DBOP_JUMP_ON_CONFLICT(ret, retry);
			ftfs_bstore_txn_abort(txn);
		} else {
...
ftfs_lookup(struct inode *dir, struct dentry *dentry, unsigned int flags)
{
	...
	if (r == -ENOENT) {
	//ftfs_error(__func__, "lookup - NOENT\n");
		inode = NULL;
		dbt_destroy(&meta_dbt);
		goto commit;
	} else if (r) {
abort:
	ftfs_error(__func__, "lookup - ABORT\n");
		inode = ERR_PTR(r);
		ftfs_bstore_txn_abort(txn);
		dbt_destroy(&meta_dbt);
		goto out;
...
static int ftfs_setattr(struct dentry *dentry, struct iattr *iattr)
{
	...
	if ((iattr->ia_valid & ATTR_SIZE) && iattr->ia_size < size) {
		...
		ftfs_error(__func__, "장난 똥떄리냐 path:%s block_num:%d, block_off:%d, size:%d modified size: %d\n", dentry->d_name.name, block_num, block_off, size, iattr->ia_size);
...
#ifdef DISABLE_DCACHE
	ftfs_bstore_meta_put_cache(sbi->meta_db, meta_dbt, txn, &meta, NULL, false);
	ftfs_put_read_lock(FTFS_I(inode));
	return 0;
#endif
	TXN_GOTO_LABEL(retry);
	ftfs_bstore_txn_begin(sbi->db_env, NULL, &txn, TXN_MAY_WRITE);
	ret = ftfs_bstore_meta_put(sbi->meta_db, meta_dbt, txn, &meta, NULL, false); // TODO: have to know parents inode
	//ftfs_bstore_meta_put_cache(sbi->meta_db, meta_dbt, txn, &meta, NULL, false);
	if (ret) {
		DBOP_JUMP_ON_CONFLICT(ret, retry);
		ftfs_bstore_txn_abort(txn);
...
static void ftfs_evict_inode(struct inode *inode)
{
	...
	ftfs_error(__func__, "\n");
#endif
	//pr_info("쫒겨난다: %d %s\n", inode->i_nlink, S_ISDIR(inode->i_mode)?"dir":"data");
	//tmp = &(FTFS_I(inode)->meta_dbt);

	//print_key(__func__, tmp->data, tmp->size);
	if (inode->i_nlink || (FTFS_I(inode)->ftfs_flags & FTFS_FLAG_DELETED)) {
		//pr_info("쫒겨난다: %d\n", inode->i_nlink);
		ftfs_bstore_meta_del(sbi->cache_db, &(FTFS_I(inode)->meta_dbt), NULL, 1, S_ISDIR(inode->i_mode));
                 ...
                 struct ftfs_metadata *meta)
{
	...
	if (!(i->i_state & I_NEW)) {
		DBT *old_dbt;
		ftfs_put_read_lock(ftfs_inode);
	       	old_dbt = ftfs_get_write_lock(ftfs_inode);
		//pr_info("잘봐라\n");
	...
	spin_lock_init(&ftfs_inode->reada_spin);
	INIT_LIST_HEAD(&ftfs_inode->ra_list);
	ftfs_inode->ra_entry_cnt = 0;
	ftfs_inode->last_block_start = ftfs_inode->last_block_len = 0;
	ftfs_inode->is_lookuped = 0;
	//init_rwsem(&ftfs_inode->reada_spin);
#endif
...
static int ftfs_fill_super(struct super_block *sb, void *data, int silent)
{
	...
	pr_info("ftfs inode size: %d, ftfs metadata size: %d\n", sizeof(struct ftfs_inode), sizeof(struct ftfs_metadata));
