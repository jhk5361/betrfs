	...
	*txn = kmem_cache_alloc(lightfs_txn_cachep, GFP_NOIO);
	...
	if (flags == TXN_SYNC_WRITE) {
		...
		if (txn_hdlr->txn_cnt == 0)
			ftfs_error(__func__, "SYNC %d, flag: %d\n", txn_hdlr->txn_cnt, (*txn)->state);
		spin_unlock_irqrestore(&txn_hdlr->txn_spin, irqflags);
	} else {
		spin_lock_irqsave(&txn_hdlr->txn_spin, irqflags);
		txn_hdlr->txn_cnt++;
		...
		(*txn)->txn_id = txn_hdlr->txn_id++;
		if (txn_hdlr->txn_cnt == 0)
			ftfs_error(__func__, "NOSYNC %d, flag: %d\n", txn_hdlr->txn_cnt,(*txn)->state);
		spin_unlock_irqrestore(&txn_hdlr->txn_spin, irqflags);
	}
...
int lightfs_bstore_txn_get_multi_reada(DB *db, DB_TXN *txn, DBT *key, uint32_t cnt, void *extra, enum lightfs_req_type type)
{
	...
	//ftfs_error(__func__, "block_num: %d, cnt: %d\n", block_num, cnt);
	txn_buf_setup(txn_buf, buf, 0, cnt, type);
	ftfs_data_key_set_blocknum(data_key, key->size, block_num);
	txn_buf->buf -= (tmp * PAGE_SIZE);
...
int lightfs_bstore_dbc_cursor(DB *db, DB_TXN *txn, DBC **dbc, enum lightfs_req_type type)
{
	...
	lightfs_txn_buf_init(txn_buf);
	txn_buf->txn_id = txn->txn_id;
	//txn_buf->key = kmalloc(META_KEY_MAX_LEN, GFP_NOIO); //TODO:META_KEY_MAX_KEY
	//txn_buf->completionp = kmem_cache_alloc(lightfs_completion_cachep, GFP_NOIO);
	//lightfs_completion_init(txn_buf->completionp);
...
static int lightfs_c_txn_insert(DB_C_TXN *c_txn, DB_TXN *txn)
{
	...
	if (txn_hdlr->txn_cnt == 0) {
		WARN_ON(1);
		ftfs_error(__func__, "TXN CNT: %ld, c_txn->size:%d, c_txn->state: %d\n", txn_hdlr->txn_cnt, c_txn->size, c_txn->state);
	}
	...
	if (txn->cnt == 0) {
		...
		if (list_empty(&txn->txn_buf_list)) {
			ftfs_error(__func__, "!!!!!!!!!!!!!!!TXN CNT: %ld, c_txn->size:%d, c_txn->state: %d\n", txn_hdlr->txn_cnt, c_txn->size, c_txn->state);
		}
...
int lightfs_bstore_c_txn_commit_flush(DB_C_TXN *c_txn) {
	...
	/*
	if (c_txn == txn_hdlr->sync_c_txn) {

	ftfs_error(__func__, "sync c_txn_commit_flush c_txn_tid:%d c_txn:%p size:%d txn_cnt: %ld running_cnt:%d committing_cnt: %d, O:%d, NO: %d\n", c_txn->txn_id, c_txn, c_txn->size, txn_hdlr->txn_cnt, txn_hdlr->running_c_txn_cnt, c_txn->committing_cnt, txn_hdlr->ordered_c_txn_cnt, txn_hdlr->orderless_c_txn_cnt);
	} else {
	ftfs_error(__func__, "c_txn_commit_flush c_txn_tid:%d c_txn:%p size:%d txn_cnt: %ld running_cnt:%d committing_cnt: %d, O:%d, NO: %d\n", c_txn->txn_id, c_txn, c_txn->size, txn_hdlr->txn_cnt, txn_hdlr->running_c_txn_cnt, c_txn->committing_cnt, txn_hdlr->ordered_c_txn_cnt, txn_hdlr->orderless_c_txn_cnt);
...
static int lightfs_c_txn_transfer(DB_C_TXN *c_txn)
{
	...
	//ftfs_error(__func__, "c_txn_transfer c_txn_tid:%d c_txn:%p size:%d txn_cnt: %d txn->cnt: %d running_cnt:%d workq_id: %d total: %d\n", c_txn->txn_id, c_txn, c_txn->size, txn_hdlr->txn_cnt, c_txn->cnt, txn_hdlr->running_c_txn_cnt, c_txn->workq_id, txn_hdlr->ordered_c_txn_cnt + txn_hdlr->orderless_c_txn_cnt);
...
static bool lightfs_txn_hdlr_check_state(void)
{
	...
	if ( cnt > 0) {
		...
		if (!list_empty(&txn_hdlr->sync_txn_list)) {
			ret = true;
			flag = 2;
                        //txn = list_first_entry(&txn_hdlr->sync_txn_list, DB_TXN, txn_list);
	...
	spin_unlock(&txn_hdlr->txn_spin);
	if (flag > 1)
		pr_info("ret: %d, flag: %d, txn_cnt: %ld\n", ret, flag, cnt);
	return ret;
}
...
int lightfs_txn_hdlr_run(void *data)
{
	DB_C_TXN *c_txn;
	DB_TXN *txn, *sync_txn; 
	DB_TXN *old_txn = NULL, *old_sync_txn = NULL; 
	//DB_C_TXN_LIST *committed_c_txn_list;
	int ret;
	//int uncommitted = 0;
...
txn_repeat:
		...
		*/
		...
		while (!list_empty(&txn_hdlr->sync_txn_list)) {
			...
			spin_lock_irqsave(&txn_hdlr->txn_spin, flags);
			//ftfs_error(__func__, "SYNC TXN 비었다 cnt:%d\n", txn_hdlr->txn_cnt);
		}
