...
{
#ifdef EMULATION
	db_env_set_default_bt_compare(env, key_ops->key_cmp); 
#endif
	return 0;
}
...
int lightfs_db_env_change_fsync_log_period (DB_ENV *env, uint32_t a)
{
	return 0;
}
...
int lightfs_db_env_get_engine_status_num_rows (DB_ENV *env, uint32_t a, uint32_t b, uint64_t c)
{
	return 0;
}
