...
static int lightfs_cache_open (DB *db, DB_TXN *txn, const char *file, const char *database, DBTYPE type, uint32_t flag, int mode)
{
	return 0;
}

static int lightfs_cache_get (DB *db, DB_TXN *txn, DBT *key, DBT *value, enum lightfs_req_type type)
{
	return db_cache_get(db, txn, key, value, 0);
}

static int lightfs_cache_put (DB *db, DB_TXN *txn, DBT *key, DBT *value, enum lightfs_req_type type)
{
	return db_cache_put(db, txn, key, value, 0);
}

static int lightfs_cache_del (DB *db , DB_TXN *txn, DBT *key, enum lightfs_req_type type)
{
	return db_cache_del(db, txn, key, 0);
}

static int lightfs_cache_weak_del (DB *db , DB_TXN *txn, DBT *key, enum lightfs_req_type type)
{
	return db_cache_weak_del(db, txn, key, 0);
}

static int lightfs_cache_close(DB *db, uint32_t flag)
{
...
/*
static inline void free_rb_tree(struct rb_node *node)
{
...
{
	struct rb_node *node = dir_ht_item->dcache->rb_root.rb_node;

	while (node) {
		struct ht_cache_item *tmp = container_of(node, struct ht_cache_item, rb_node);
		int result;

		result = lightfs_keycmp(tmp->key.data, tmp->key.size, key->data, key->size);
...
{
	struct ht_cache_item *cache_item, *dir_cache_item;
	...
	//cache_item = lightfs_dcache_find_val_with_key(dir_cache_item, &(node->key));
	//if (cache_item == NULL) {
...
{
	//rb_erase(&node->rb_node, &(dir_ht_item->dcache->rb_root));
	//static int a = 0;
	//BUG_ON(node->parent == NULL);
	//BUG_ON(node->parent->dcache == NULL);
	//BUG_ON(&(node->parent->dcache->rb_root) == NULL);
	//if (a++ == 0)
...
//int (*cache_put) (DB *, DB_TXN *, DBT *, DBT *, enum lightfs_req_type, struct ftfs_inode *dir_f_inode, bool is_dir);
//
int lightfs_ht_cache_group_eviction (DBT *key)
{
	struct ht_lock_item *ht_item;
	struct ht_cache_item *cache_item, *child_cache_item;
	DBT *dir_key;
	uint32_t dir_hkey, dir_fp;
	struct ftfs_inode *dir_f_inode;
	uint32_t hkey = lightfs_ht_func(FSEED, key->data, key->size);
	uint32_t fp = lightfs_ht_func(SSEED, key->data, key->size);
	...
	if (child_cache_item) {
		...
		while (rb_node) {
			...
			ret = ftfs_bstore_meta_put_tmp(NULL, &child_cache_item->key, txn, child_cache_item->value.data, NULL, 0);
...
static int lightfs_ht_cache_put (DB *db, DB_TXN *txn, DBT *key, DBT *value, enum lightfs_req_type type, struct inode *dir_inode, bool is_dir)
{
	...
	hash_for_each_possible(lightfs_ht_lock, ht_item, hnode, hkey) {
		//pr_info("들어간다2 %p\n", &ht_item->lock);
		//print_key(__func__, key->data, key->size);
		//ftfs_error(__func__, "key_size: %d, hkey: %d, fp: %d\n", key->size, hkey, fp);
		//spin_lock_bh(&ht_item->lock);
		//spin_lock(&ht_item->lock);
		down_write(&ht_item->lock);
		hash_for_each_possible(lightfs_ht_cache, cache_item, hnode, hkey) {
	...
	if (dir_key) {
		...
		//ftfs_error(__func__, "DIR key_size: %d, hkey: %d, fp: %d\n", key->size, dir_hkey, dir_fp);
...
static int lightfs_dcache_c_get(DBC *c, DBT *key, DBT *value, uint32_t flags)
{
		...
		hash_for_each_possible(lightfs_ht_lock, ht_item, hnode, hkey) {
			...
			if (cache_item) { // found directory
				...
				if (rb_node) {
					...
					//ftfs_error(__func__, "SET_RANGE 못찾음\n");
	...
	//key->data = wrap->node->key.data;
	//key->size = wrap->node->key.size;
	//value->data = wrap->node->value.data;
